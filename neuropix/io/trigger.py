...
def get_trigger_data_sync_events(folder_path):
    """
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.txt'):
            if 'inv' in filename:
                ...
                arr = read_file(os.path.join(folder_path, filename))
                if arr:
...
def get_trigger_data(folder_path):
    """
    Reads all trigger data files in a folder and returns a dictionary of trigger data.
...
def timestamps_to_binary_array(channels_dict, timesteps):
